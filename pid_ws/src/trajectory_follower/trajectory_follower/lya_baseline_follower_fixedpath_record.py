...
CSV_PATH = "/home/nvidia/pid_ws/src/trajectory_follower/trajectory_follower/course0611_inverted.csv"
LD = 2.2      # 前视距离 (m)
DT = 0.1          # 控制周期 (s)
...
class Follower(Node):
    def __init__(self):
        ...
        self.data = pd.DataFrame(columns=["Time", "V", "Omega","X","Y","theta", "X_current", "Y_current", "theta_current", "dx", "dy", "LAM_V", "LAM_A", "K1", "K2"])

        # 3) 文件夹路径和Excel文件名，可自定义
        self.folder_path = "./lane_analysis_data"
        if not os.path.exists(self.folder_path):
            os.makedirs(self.folder_path)
        self.excel_file_path = os.path.join(self.folder_path, "lya_fixedpath_data.xlsx")
        # 定时器
    ...
    def control_loop(self):
        ...
        omega = alpha/(2 * alpha - beta) * (LAM_A * alpha + (V_T / r) * (math.sin(2 * alpha) * math.cos(beta)/2 - math.sin(beta)) + math.sin(2 * alpha) * LAM_V / 2)
        + (alpha - beta) * omega_des / (2 *alpha - beta)
        ...
        new_row = pd.DataFrame([[current_time_str, v, omega, target_x, target_y, phi_t, current_x, current_y, current_yaw, dx, dy, LAM_V, LAM_A]],
                                columns=["Time", "V", "Omega","X","Y","theta", "X_current", "Y_current", "theta_current", "dx", "dy", "LAM_V", "LAM_A"])
