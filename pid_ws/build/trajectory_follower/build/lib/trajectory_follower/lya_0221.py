...
class TrajectoryFollower(Node):
    def __init__(self):
        ...
        # ---- (A) 订阅 '/filtered_omega_t' 以获得 omega_t，不对其做饱和限制 ----
        ...
        # 此时 self.lya.omega_t 保持外部输入的原始值
    ...
    def omega_t_callback(self, msg):
        """
        这里的信息中只有 theta 部分表示需要的 omega_t，不对其饱和限制
        """
    ...
    def trajectory_callback(self, msg):
        ...
        alpha = math.atan2(math.sin(alpha), math.cos(alpha))  # 归一化

        beta = theta - phi_t
        beta = math.atan2(math.sin(beta), math.cos(beta))     # 归一化
