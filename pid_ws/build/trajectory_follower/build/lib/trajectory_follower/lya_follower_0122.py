...
class TrajectoryFollower(Node):
    def __init__(self):
        ...
        self.data = pd.DataFrame(columns=["Time", "V", "Omega"])
    ...
    def trajectory_callback(self, msg):
        ...
        # 记录数据
        # timestamp = time.strftime("%H:%M:%S", time.localtime(time.time()))  # 24 小时制
        # self.data_v_t.append((timestamp, v))
        # self.data_omega_t.append((timestamp, omega))
    ...
    def save_data_to_excel(self):
        ...
        lya_file_path = f"{folder_path}/lya_data.xlsx"
        ...
        try:
            ...
            self.data.to_excel(lya_file_path, index=False)
            self.get_logger().info(f"Data saved to {lya_file_path}")
        except Exception as e:
