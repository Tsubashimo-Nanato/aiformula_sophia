...
void LanePixelFinder::searchMask(const cv::Mat& binary_mask, LaneLines& lane_lines) const {
    const int cols = binary_mask.cols;  // 图像宽
    const int rows = binary_mask.rows;  // 图像高

    // 我们从图像下方（bottom）往上（top）遍历
    const int top = 0;
    const int bottom = rows - 1;

    // 以图像水平中心(cx)为基准，定义一个半宽度(half_width)，将图像大致分成左、中、右三块
    const int cx = cols / 2;       // 水平中心列
    const int half_width = 100;    // 中心区域左右各拓展100像素
    const int center_left  = std::max(0,        cx - half_width);
    const int center_right = std::min(cols - 1, cx + half_width);

    // 逐行扫描
    for (int row = bottom; row >= top; --row) {
        const uchar* row_ptr = binary_mask.ptr<uchar>(row);

        // 1) 中间区域搜索
        bool found_center = false;
        int  center_col   = -1;
        // 在 [center_left, center_right] 范围搜索第一个为1的像素
        for (int col = center_left; col <= center_right; ++col) {
            if (row_ptr[col]) {
                found_center = true;
                center_col   = col;
                break;
        ...
        if (found_center) {
            // 如果找到了，就存到 lane_lines.center
            lane_lines.center.pixels.emplace_back(center_col, row);
        }

        // 2) 左侧区域搜索（可选，如果不想要就删除/注释掉此段）
        bool found_left = false;
        int  left_col   = -1;
        // 在 [0, center_left-1] 范围从右往左搜
        for (int col = center_left - 1; col >= 0; --col) {
            if (row_ptr[col]) {
                found_left = true;
                left_col   = col;
                break;
        ...
        if (found_left) {
            lane_lines.left.pixels.emplace_back(left_col, row);
        }

        // 3) 右侧区域搜索（可选，如果不想要就删除/注释掉此段）
        bool found_right = false;
        int  right_col   = -1;
        // 在 [center_right+1, cols-1] 范围从左往右搜
        for (int col = center_right + 1; col < cols; ++col) {
            if (row_ptr[col]) {
                found_right = true;
                right_col   = col;
                break;
        ...
        if (found_right) {
            lane_lines.right.pixels.emplace_back(right_col, row);
        }
    }
}
