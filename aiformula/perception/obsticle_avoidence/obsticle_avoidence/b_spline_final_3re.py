...
class RedAlertDetector(Node):
    def __init__(self):
        ...
        self.red_thresh_px   = self.declare_parameter('red_thresh_px', 1000).value
        self.red_resume_px   = self.declare_parameter('red_resume_px', 100).value
        ...
        # ROI
        self.trap_top_ratio  = self.declare_parameter('trap_top_ratio', 0.5).value
        self.trap_left_ratio, self.trap_right_ratio = 0.373, 0.625
        self.trap_bottom_left_ratio, self.trap_bottom_right_ratio = 0.15, 0.85

        # 红色 HSV 阈值
        self.lower_red = np.array([0, 120, 70]); self.upper_red = np.array([195, 255, 255])
    ...
    def img_cb(self,msg:Image):
        ...
        if rect_red>self.red_thresh_px:
            ...
            self.compute_and_store_path(col_mask,vis,mask)            # 也绘制通用叠加（中心线/ROI/文字统计），避免只显示路径时其他元素消失
            self.draw_common(vis, roi_cnt, True, rect_red, int(cv2.countNonZero(mask)))
